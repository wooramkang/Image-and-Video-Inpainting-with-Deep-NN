...
def CN3D(input_video = None, sampling_frame= 8,  vid_net_mid_depth = 3):
    ...
    d0_CC = Conv3D(filters=512,padding='same', kernel_size=3)(d0_CC)
    d0_CC = Bat(d0_CC)
    d0_CC = Activ(d0_CC)
    ...
    d1 = UpSampling3D()(d1)
    d1 = Conv3D(strides=1, filters=256, kernel_size= 4, padding='same')(d1)
    d1 = Bat(d1)
    d1 = Activ(d1)
    #print(d1.get_shape())

    d1_C = Concatenate()([d1, e1])    
    d1_C = Conv3D(filters=128,padding='same', kernel_size=3)(d1_C)
    d1_C = Bat(d1_C)
    d1_C = Activ(d1_C)
    ...
    d1_CC = Conv3D(strides=1, filters=64, kernel_size= 4, padding='same')(d1_CC)
    d1_CC = Bat(d1_CC)
    d1_CC = Activ(d1_CC)
