...
def train_one_epoch(mask_loader, train_dataloader, BATCH_SIZE, check_resize = True):
    ...
    pd_loss = pdCN_model.predict_on_batch ( [img_masked_batch, mask_batch ], img_train_batch )

    return pd_loss

def test_one_epoch(mask_loader, train_dataloader, val_dataloader, BATCH_SIZE, check_resize = True):
...
def train():
    ...
    for i in range(EPOCH):
        ...
        if i % SAVE_TERM_PER_EPOCH == 0:
            ...
            masked_in, result, raw_img = test_one_epoch(mask_loader, train_dataloader_forward, val_dataloader_forward, BATCH_SIZE)   
            fig = plt.figure()
            rows = BATCH_SIZE
            cols = 3
            c = 0
        ...
        print("train forward")
        forward_loss = train_one_epoch(mask_loader, train_dataloader_forward, BATCH_SIZE, FRAME_SIZE)    
